...
def is_bid_valid(bid_form, listing_id, user):
    ...
    max_bid = Bid.objects.filter(
        listing=listing_id).aggregate(Max("bid_price"))['bid_price__max']
    max_bidder = Bid.objects.get(listing=listing_id, bid_price=max_bid).bidder
    lister = Listing.objects.get(id=listing_id).lister

    if bid <= max_bid:
        message = f"Top bid is ${max_bid:.2f}. Your ${bid} bid must be higher."
    elif bidder == max_bidder:
        message = "You cannot raise your own top bid."
    elif bidder == lister:
